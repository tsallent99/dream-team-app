...
import type { StoreApi } from "../../lib/zustand";
import { createDummyStore, isStoreApi } from "../../lib/zustand";
...
export type GenericContextProviderBuilder<T> = (
  value: T | StoreApi<T>
) => ProviderWithValue<T>;
...
const getUnsupportedResetValue = (contextName: string) => () => {
  throw new Error(
    `Resetting value of context '${contextName}' is not supported`
  );
};
 ...
 *
 * If the context is not set, error will be thrown upon accessing it using the exposed hook. (unless context was marked as optional).
 *
 * @param opts - options
 */
export const buildContext = <T,>(
  ...
  const useGenericContext = (): ContextValue<T> => {
    ...
    // Store library adaptation: Wraps the store value in order to use library-specific hook
    const dummyStore = createDummyStore(contextValue.value);
    const storeState = useStore(
      isStoreApi<T>(contextValue.value) ? contextValue.value : dummyStore
    );
    let storageValue = null;

    if (isStoreApi<T>(contextValue.value)) {
      storageValue = { value: storeState } as ContextValue<T>;
    }

    if (!contextValue && !opts?.optional) {
      throw new Error(
        `Context value '${ctxName}' must be set and within a Provider`
      );
    }
    return storageValue ?? (contextValue as ContextValue<T>);
  };
...
export const createWritableGenericContext = <T,>(
  ...
  buildContext<T>({ writable: true, contextName });
